...
class _LogScreenState extends State<LogScreen> {
  ...
  List<CSVLogModel> get _allLogs => AnalyticService.instance.logList;

  // logs sau filter + sort
  List<CSVLogModel> _filteredLogs = [];
  ...
  //             style: const TextStyle(fontSize: 9, color: PdfColors.grey600),
  //           ),
  //         );
  //       },
  //
  ...
  //             // message rộng hơn
  //             columnWidths: const {
  //               0: FlexColumnWidth(3), // user_id
  //               1: FlexColumnWidth(2), // created_at
  //               2: FlexColumnWidth(2), // gen_time
  //               3: FlexColumnWidth(7), // message
  //             },
  //           ),
  //         ];
  //       },
  ...
  void _rebuildFilteredLogs() {
    ...
    List<CSVLogModel> tmp = _allLogs.where((log) {
      if (searchLower.isNotEmpty) {
        final msg = log.message?.toLowerCase();
        final uid = log.userId?.toLowerCase();

        if (!(msg?.contains(searchLower) ?? false) && !(uid?.contains(searchLower) ?? false)) {
          return false;
        }
      }

      if(log.genTime == null) return false;
      if (fromEpoch != null && log.genTime!.millisecondsSinceEpoch < fromEpoch) return false;
      if (toEpoch != null && log.genTime!.millisecondsSinceEpoch > toEpoch) return false;
  ...
  Future<void> _pickFromGenDateTime() async {
    ...
    final initial =
        _fromGenDateTime ??
            (_allLogs.isNotEmpty ? _allLogs.first.genTime : now);
  ...
  Future<void> _pickToGenDateTime() async {
    ...
    final initial =
        _toGenDateTime ??
            (_allLogs.isNotEmpty ? _allLogs.last.genTime : now);
  ...
  Widget build(BuildContext context) {
    ...
    final int endIndex;
    List<CSVLogModel> pageLogs;

    if (total == 0) {
      startIndex = 0;
      endIndex = 0;
      pageLogs = <CSVLogModel>[];
    } else {
  ...
  Widget _buildPagedTable({
    ...
    required List<CSVLogModel> pageLogs,
    required int total,
